...
class AvatarUpdateView(UpdateView):
    ...
    def get_object(self, queryset=None):
        #Crea el avatar asociado al usuario actual
        avatar, created = Avatar.objects.get_or_create(user=self.request.user)
        return avatar

class ProfileUpdateView(LoginRequiredMixin, View):
    template_name = "usuarios/edit_profile.htmal"
    success_url = reverse_lazy("profile")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "update_profile" in request.POST:
            # Se envio el formulario para actualizar datos y avatar
            if user_form.is_valid() and avatar_form.is_valid():
                user_form.save()
                avatar_form.save()
                return redirect(self.success_url)
        elif "change_password" in request.POST:
            # Se envio el formulario para cambiar contraseña
            if password_form.is_valid():
                user = password_form.save()
                # Importante: actualizar la sesion para que el usuario no se desconecte
